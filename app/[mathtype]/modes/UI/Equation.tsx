...
const Equation: FC<IProps> = ({ equation, res, currentPage }) => {
  ...
  const [bgColor, setBgColor] = useState<
    'bg-white' | 'bg-red-500' | 'bg-green-500'
  >('bg-white')
  const [isShowButtons, setShowButtons] = useState(true)

  const [flag, count, diff] = useEquationStore(
    state => [state.trigger, state.count, state.diff],
    shallow
  )
  const [addAns, addCorrAns, remCorrAns] = useEquationResultsStore(
    state => [
      state.addAnsweredCount,
      state.addCorrectlyAnsweredCount,
      state.removeCorrectlyAnsweredCount,
    ],
    shallow
  )
  useEffect(() => {
  ...
  return (
    <div className="my-2 flex gap-1 items-center h-[50px] sm:flex-col sm:gap-2 sm:h-[65px]">
      <div className="my-2 flex gap-1 items-center h-[50px]">
        <div className="pr-1">
          <div
            className={`border-2 border-black rounded-full w-[15px] h-[15px] ${bgColor}`}></div>
        </div>
        <div
          className="select-none h-min cursor-pointer font-[600]"
          onClick={setShow}>
          {equation}
        </div>
        {isShowed && (
