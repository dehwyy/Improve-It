...
const Mode: FC<IProps> = ({ currentPage }) => {
  const [diff, count, trigger] = useEquationStore(
    state => [state.diff, state.count, state.flag],
    shallow
  )
  ...
  const equations = useMemo(
    () => Array.from(getEquations(currentPage, diff, count)),
    [diff, count, trigger]
  )
  ...
  return equations && equations[0][0] ? (
    <>
      ...
      <div className="text-end pr-5">
        <Button variant="contained" sx={btnWhiteTheme}>
          Submit score
        </Button>
      </div>
    </>
    ...
    <div className="text-center" style={{ wordSpacing: '7px' }}>
      ...
      I would be UwU {`${'if'}`} you decide to help me with making hard
      equations or maybe something like derivative, integral calculation or
