...
class EquationInjection implements EquationDiffs {
  constructor(private EqGenerator: EquationDiffs) {}
...
export default function* getEquations(mode: Modes, diff: Diffs = 0, count = 1): Generator<EquationT, void> {
  let localCount = 0
  const Eqs = mode == Modes.speed ? new EquationInjection(new EquationSpeed()) : new EquationInjection(new EquationHard())
